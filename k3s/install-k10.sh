...
function ensure_k10_image() {
    ...
    if [ "$K10_USE_CUSTOM_REGISTRY" == "y" ]; then
        if deploy_image_pull "$IMAGE" "$K10_REGISTRY_USERNAME" "$K10_REGISTRY_TOKEN"; then
            log_info "image is pullable: $IMAGE"
            return
        fi
        buildkit_ensure_client /usr/local/bin
        buildkit_build "$CONTEXT" "$IMAGE" "$K10_BUILDKIT_ADDR" "temp/k10-docker-config" "$BASE_IMAGE"
    else
...
kube_secret_load "$NS" "k10-cluster-passphrase" passphrase=PASSWD
if [ -z "$PASSWD" ]; then
    PASSWORD_SEED=$(prompt_required "please input admin password seed for k10." "password seed" "")
    PASSWD=$(password_derive_sha1 "$PASSWORD_SEED@k10" 32)
fi
kube_secret_load "$NS" "k10-custom-images" \
...
if [ "$INSTALL_MODE" == "reinstall" ]; then
    helm_ensure_chart "kasten" "https://charts.kasten.io/" "k10" "temp"
    VERSION_PAIR=$(helm_chart_versions_local "temp/k10")
    read -r CHART_VERSION VERSION <<<"$VERSION_PAIR"
else
    VERSION_PAIR=$(helm_chart_versions "kasten" "https://charts.kasten.io/" "k10")
    read -r CHART_VERSION DEFAULT_VERSION <<<"$VERSION_PAIR"
    VERSION=$(prompt_with_default "please input k10 version." "version" "$DEFAULT_VERSION")
    if [ "$VERSION" != "$DEFAULT_VERSION" ]; then
        VERSION_PAIR=$(helm_chart_versions "kasten" "https://charts.kasten.io/" "k10" "$VERSION")
...
if [ "$K10_USE_CUSTOM_REGISTRY" == "y" ]; then
    ...
    [ -n "$K10_BUILDKIT_ADDR" ] || K10_BUILDKIT_ADDR=$(prompt_with_default "" "buildkit address" "tcp://buildkit.$DOMAIN:1234")
    K10_REGISTRY_HOST=$(registry_host "$K10_REGISTRY_URL")
else
...
K10_KANISTER_TOOLS_IMAGE_PATH=$(registry_strip_host "$K10_KANISTER_TOOLS_IMAGE")
K10_METERING_IMAGE_PATH=$(registry_strip_host "$K10_METERING_IMAGE")
...
deploy_render_values values-k10.yaml
kube_secret_apply_vars "$NS" "k10-cluster-passphrase" passphrase=PASSWD
kube_secret_apply_vars "$NS" "k10-dr-secret" key=PASSWD
kube_secret_apply_vars "$NS" "kopia-repo-password" password=PASSWD
kube_secret_apply_vars "$NS" "k10-custom-images" \
    use-custom-registry=K10_USE_CUSTOM_REGISTRY \
    registry-url=K10_REGISTRY_URL \
    registry-username=K10_REGISTRY_USERNAME \
    registry-token=K10_REGISTRY_TOKEN \
    ...
    license=K10_LICENSE_VALUE
if [ "$K10_USE_CUSTOM_REGISTRY" == "y" ]; then
    kube_apply_registry_secret "$NS" "k10-custom-registry" "$K10_REGISTRY_HOST" "$K10_REGISTRY_USERNAME" "$K10_REGISTRY_TOKEN"
    registry_write_auth "temp/k10-docker-config" "$K10_REGISTRY_HOST" "$K10_REGISTRY_USERNAME" "$K10_REGISTRY_TOKEN"
fi
...
# install
helm_ensure_chart "kasten" "https://charts.kasten.io/" "k10" "temp" "$CHART_VERSION"
...
EOF
TOKEN=$(kube_secret_get "kasten-io" "my-k10-admin" "token")
log_trace "\nk10 admin token: \n"
log_reminder "$TOKEN \n"
