...
helm upgrade --install -n $NS postgresql temp/postgresql -f temp/values-postgresql.yaml \
...
helm upgrade --install -n $NS redis temp/redis -f temp/values-redis.yaml \
...
helm upgrade --install -n $NS nextcloud temp/nextcloud -f temp/values-nextcloud.yaml --set replicaCount=1
k8s_wait $NS deployment nextcloud 200
...
helm upgrade --install -n $NS office temp/app-template -f temp/values-office.yaml
