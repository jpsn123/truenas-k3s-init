...
WORKSPACE_REGISTRY_HOST=$(registry_host "$WORKSPACE_REGISTRY_URL")
WORKSPACE_IMAGE_PATH=$(registry_strip_host "$WORKSPACE_IMAGE_PATH")
...
kube_secret_apply_vars "$NS" "coder-workspace-image-registry" \
    registry-url=WORKSPACE_REGISTRY_URL \
    registry-username=WORKSPACE_REGISTRY_USERNAME \
    registry-token=WORKSPACE_REGISTRY_TOKEN
kube_secret_apply_vars "$NS" "coder-workspace-image-build" \
    config-version=BUILD_CONFIG_VERSION \
    buildkit-addr=BUILDKIT_ADDR \
    image-path=WORKSPACE_IMAGE_PATH \
    ubuntu-version=UBUNTU_VERSION \
    ...
    web-tag=WEB_TAG \
    platforms=PLATFORMS
registry_write_auth "$DOCKER_CONFIG_DIR" "$WORKSPACE_REGISTRY_HOST" "$WORKSPACE_REGISTRY_USERNAME" "$WORKSPACE_REGISTRY_TOKEN"
