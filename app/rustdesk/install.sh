...
NS=rustdesk
INSTALL_MODE="${1:-full}"
deploy_validate_mode "$INSTALL_MODE" rustdesk
...
if deploy_mode_enabled "$INSTALL_MODE" rustdesk; then
    ...
    if [ "$RUSTDESK_USE_CUSTOM_REGISTRY" == "y" ]; then
        ...
        [ -n "$RUSTDESK_BUILDKIT_ADDR" ] || RUSTDESK_BUILDKIT_ADDR=$(prompt_with_default "" "buildkit address" "tcp://buildkit.$DOMAIN:1234")
    else
    ...
    RUSTDESK_REGISTRY_HOST=$(registry_host "$RUSTDESK_REGISTRY_URL")

    RUSTDESK_IMAGE_PATH=$(registry_strip_host "$RUSTDESK_IMAGE_REPOSITORY")
    ...
    kube_secret_apply_vars "$NS" "rustdesk-custom-image" \
        use-custom-registry=RUSTDESK_USE_CUSTOM_REGISTRY \
        registry-url=RUSTDESK_REGISTRY_URL \
        registry-username=RUSTDESK_REGISTRY_USERNAME \
        registry-token=RUSTDESK_REGISTRY_TOKEN \
        ...
        image-repository=RUSTDESK_IMAGE_REPOSITORY
    if [ "$RUSTDESK_USE_CUSTOM_REGISTRY" == "y" ]; then
        kube_apply_registry_secret "$NS" "rustdesk-custom-registry" "$RUSTDESK_REGISTRY_HOST" "$RUSTDESK_REGISTRY_USERNAME" "$RUSTDESK_REGISTRY_TOKEN"
        registry_write_auth "temp/rustdesk-docker-config" "$RUSTDESK_REGISTRY_HOST" "$RUSTDESK_REGISTRY_USERNAME" "$RUSTDESK_REGISTRY_TOKEN"
    fi
    if [ "$RUSTDESK_USE_CUSTOM_REGISTRY" == "y" ]; then
        if deploy_image_pull "$RUSTDESK_FULL_IMAGE" "$RUSTDESK_REGISTRY_USERNAME" "$RUSTDESK_REGISTRY_TOKEN"; then
            log_info "image is pullable: $RUSTDESK_FULL_IMAGE"
        else
            buildkit_ensure_client /usr/local/bin
            buildkit_build "image" "$RUSTDESK_FULL_IMAGE" "$RUSTDESK_BUILDKIT_ADDR" "temp/rustdesk-docker-config" "rustdesk/rustdesk-server-pro:$RUSTDESK_IMAGE_VERSION"
        fi
    elif deploy_image_pull "$RUSTDESK_FULL_IMAGE"; then
        log_info "image is pullable: $RUSTDESK_FULL_IMAGE"
    else
