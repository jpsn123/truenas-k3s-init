...
function ensure_rustdesk_image() {
    local IMAGE="$1"
    local CONTEXT="$2"
    local BASE_IMAGE="$3"

    if [ "$RUSTDESK_USE_CUSTOM_REGISTRY" == "y" ]; then
        if k3s_crictl_pull_image "$IMAGE" "$RUSTDESK_REGISTRY_USERNAME" "$RUSTDESK_REGISTRY_TOKEN"; then
            log_info "image is pullable: $IMAGE"
            return
        fi
        build_image_with_buildkit "$CONTEXT" "$IMAGE" "$RUSTDESK_BUILDKIT_ADDR" "temp/rustdesk-docker-config" "$BASE_IMAGE"
    else
        if k3s_crictl_pull_image "$IMAGE"; then
            log_info "image is pullable: $IMAGE"
            return
        fi
        kubectl -n "$NS" patch configmap "rustdesk-install-version" --type=json -p='[{"op":"remove","path":"/data/image-version"}]' >/dev/null 2>&1 || true
...
if install_mode_enabled "$INSTALL_MODE" rustdesk; then
    ...
    if [ "$INSTALL_MODE" == "reinstall" ]; then
        if [ -z "$RUSTDESK_IMAGE_VERSION" ]; then
            log_error "missing rustdesk version configmap, please run full or rustdesk mode first."
            exit 1
        fi
    else
    ...
    ensure_rustdesk_image "$RUSTDESK_FULL_IMAGE" "image" "rustdesk/rustdesk-server-pro:$RUSTDESK_IMAGE_VERSION"
fi
