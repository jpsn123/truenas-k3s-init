...
helm $METHOD -n cattle-system rancher temp/rancher -f rancher-values.yaml \
  --set hostname=${SUB_DOMAIN}.${DNS_DOMAIN} \
  --set ingress.tls.secretName=${SUB_DOMAIN}.${DNS_DOMAIN}-tls
